...
namespace Aarthificial.Typewriter.Blackboards
{
    ...
    public class Blackboard : ISerializationCallbackReceiver, IBlackboard
    {
        ...
        public void Set(int key, float value) => Set(key, BitConverter.SingleToInt32Bits(value));

        public void Set(int key, bool value) => Set(key, value ? 1 : 0);
        ...
        public void Set(int key, int value)
        {
            ...
            {
                index = List.Count;
                List.Add(new Pair { Key = key, Value = value, });
        ...
        public float GetFloat(int key) => BitConverter.Int32BitsToSingle(Get(key));

        public bool GetBool(int key) => Get(key) != 0;

        public int Get(int key) => _lookup.TryGetValue(key, out var index) ? List[index].Value : 0;
        ...
        public IBlackboard Clone() => new Blackboard { List = new List<Pair>(List), _lookup = new Dictionary<int, int>(_lookup) };
