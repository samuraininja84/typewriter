...
namespace Aarthificial.Typewriter
{
    ...
    {
        public static bool WouldInvoke(this ITypewriterContext provider, EntryReference reference) => reference.TryGetEntry(out var entry) && provider.WouldInvoke(entry);

        public static bool WouldInvoke(this ITypewriterContext provider, BaseEntry entry) => entry.Test(provider) && provider.HasMatchingRule(entry.ID);

        public static bool TryInvoke(this ITypewriterContext provider, EntryReference reference) => reference.TryGetEntry(out var entry) && provider.TryInvoke(entry);

        public static bool TryInvoke(this ITypewriterContext provider, BaseEntry entry)
        {
            if (entry.Test(provider)) return provider.Process(entry);
            return false;
        }

        public static bool Process(this ITypewriterContext provider, EntryReference reference) => reference.TryGetEntry(out var entry) && provider.Process(entry);

        public static bool Process(this ITypewriterContext provider, BaseEntry entry)
        {
            entry.Apply(provider);
            if (!provider.FindMatchingRule(entry.ID, out var match)) return false;
        ...
        {
            if (reference.TryGetEntry(out var entry) && provider.TryGetBlackboard(entry.Scope, out var blackboard))
            {
                TypewriterDatabase.Instance.MarkChange();
                blackboard.Set(entry.ID, value);
            }
        ...
        public static int Get(this ITypewriterContext provider, BaseEntry entry) => provider.TryGetBlackboard(entry.Scope, out var blackboard) ? blackboard.Get(entry.ID) : 0;

        public static int Get(this ITypewriterContext provider, EntryReference reference)
        {
            return reference.TryGetEntry(out var entry) && provider.TryGetBlackboard(entry.Scope, out var blackboard) ? blackboard.Get(entry.ID) : 0;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool Test(this ITypewriterContext provider, BlackboardCriterion criterion)
        {
            if (criterion.FactReference.TryGetEntry(out var entry) && provider.TryGetBlackboard(entry.Scope, out var blackboard))
            {
        ...
        public static bool Test(this ITypewriterContext provider, TypewriterCriteria criteria)
        {
            for (var i = 0; i < criteria.List.Length; i++)
            {
                ...
                if (!Test(provider, criterion)) return false;
            }
        ...
        {
            if (modification.FactReference.TryGetEntry(out var entry) && provider.TryGetBlackboard(entry.Scope, out var blackboard))
            {
                TypewriterDatabase.Instance.MarkChange();
        ...
        {
            if (!reference.TryGetEntry(out var entry)) return false;
            for (var i = 0; i < entry.Entries.Count; i++)
            {
                var response = entry.Entries[i];
                if (response.Test(provider)) return true;
            }
            return false;
        }
        ...
        {
            if (!reference.TryGetEntry(out var entry)) return 0;
            var count = 0;
            for (var i = 0; i < entry.Entries.Count; i++)
            {
                var response = entry.Entries[i];
                if (response.Test(provider)) count++;
      }
            return count;
        }
        ...
        public static int FindMatchingRules(this ITypewriterContext provider, EntryReference reference, BaseEntry[] matches)
        {
            ...
            {
                var response = entry.Entries[i];
                if (response.Test(provider)) matches[count++] = response;
      }
            return count;
        }
        ...
        public static int FindMatchingRules<T>(this ITypewriterContext provider, EntryReference reference, T[] matches) where T : BaseEntry
        {
            ...
            {
                var response = entry.Entries[i];
                if (response.Test(provider) && response is T typed) matches[count++] = typed;
            }
