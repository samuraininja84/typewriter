...
namespace Aarthificial.Typewriter.Entries
{
    ...
    public class RuleEntry : BaseEntry
    {
        ...
        protected static void Dispatch(ITypewriterContext context, Dispatcher[] dispatchers)
        {
            for (var i = 0; i < dispatchers.Length; i++)
            {
                var reference = dispatchers[i].Reference;
                if (reference.TryGetEntry(out var entry) && entry.Test(context)) entry.Invoke(context);
            }
        }
        ...
        public override void AddToTable(DatabaseTable table) => table.Rules.Add(this);

        public override void RemoveFromTable(DatabaseTable table) => table.Rules.Remove(this);
