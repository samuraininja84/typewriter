...
namespace Aarthificial.Typewriter.Entries
{
    ...
    public abstract class BaseEntry : IComparable<BaseEntry>
    {
        ...
        [EntryFilter(Variant = EntryVariant.Fact, BaseType = typeof(ScopeEntry), AllowEmpty = true)]
        public EntryReference Scope;

        [SerializeField]
        [InspectorName("Triggered by")]
        public TriggerList Triggers = new() { List = Array.Empty<EntryReference>() };
        ...
        [SerializeField]
        public BlackboardModification[] Modifications = Array.Empty<BlackboardModification>();

        [NonSerialized] public List<BaseEntry> Entries;

        public int Weight => Criteria.Length + Padding;

        public int CompareTo(BaseEntry other) => other.Weight.CompareTo(Weight);
        ...
        public virtual void Apply(ITypewriterContext context)
        {
            ...
            if (Scope.HasValue) context.Add(this, 1);
        }

        public virtual bool Test(ITypewriterContext context)
        {
            if (Once && Scope.HasValue && context.Get(this) != 0) return false;
            for (var i = 0; i < Criteria.Length; i++)
            {
                var criterion = Criteria[i];
                if (!context.Test(criterion)) return false;
            }
            return true;
        }

        public virtual void Invoke(ITypewriterContext context) => TypewriterDatabase.Instance.OnEntryEvent(context, this);
