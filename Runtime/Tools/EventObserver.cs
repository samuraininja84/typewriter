...
namespace Aarthificial.Typewriter.Tools
{
    public class EventObserver : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            {
              TypewriterDatabase.Instance.AddListener(_listeners[index].EventReference, _listeners[index].Invoke);
            }
        }
        ...
        private void OnDisable()
        {
            for (var index = 0; index < _listeners.Length; index++)
            {
                TypewriterDatabase.Instance.RemoveListener(_listeners[index].EventReference, _listeners[index].Invoke);
            }
        }
        ...
        private struct EventData
        {
            ...
            {
                if (_once && _wasDispatched) return;
                _wasDispatched = true;
                _unityEvent.Invoke();
            }
        }
