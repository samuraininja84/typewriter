...
namespace Aarthificial.Typewriter.Editor.Common
{
    public static class TypewriterUtils
    {
        ...
        public static void RefreshDatabase()
        {
            ...
            if (TypewriterDatabase.Instance != null) Events.OnDatabaseCreated();
        }
        ...
        public static string GetKey(this BaseEntry entry)
        {
            ...
            {
                var tableCollection = UnityEditor.Localization.LocalizationEditorSettings.GetStringTableCollection(textRule.Text.TableReference);
                if (tableCollection == null) return "<empty>";
                return tableCollection.SharedData.GetKey(textRule.Text.TableEntryReference.KeyId) ?? "<empty>";
            }
      #endif
        ...
        internal static IEnumerable<T> GetMatching<T>(this T filter) where T : Enum
        {
            foreach (T value in Enum.GetValues(typeof(T)))
            {
                if (filter.HasFlag(value)) yield return value;
            }
        }
