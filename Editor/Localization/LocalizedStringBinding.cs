...
namespace Aarthificial.Typewriter.Editor.Localization
{
    public class LocalizedStringBinding : IBinding
    {
        ...
        {
            _parent = parent;
            LocalizationEditorSettings.EditorEvents.TableEntryModified += HandleTableEntryModified;
        }
        ...
        public SerializedProperty Property
        {
            set
            {
                _collectionName = value?.FindPropertyRelative("m_TableReference").FindPropertyRelative("m_TableCollectionName");
                _entryKeyId = value?.FindPropertyRelative("m_TableEntryReference").FindPropertyRelative("m_KeyId");
                Update();
            }
        }
        ...
        public string Value
        {
            ...
            set
            {
                if (_table == null || _entry == null || _sharedTableEntry == null) return;
        ...
        public void Update()
        {
            if (_lastEntryKeyId == _entryKeyId?.longValue && _lastCollectionName == _collectionName?.stringValue && _lastEntryKey == Key) return;

            _lastEntryKeyId = _entryKeyId?.longValue ?? SharedTableData.EmptyId;
            _lastCollectionName = _collectionName?.stringValue;

            var tableCollection = LocalizationEditorSettings.GetStringTableCollection(GetTableReference());
        ...
        private TableReference GetTableReference()
        {
            ...
            bool hasGuidTag = !string.IsNullOrEmpty(value) && value.StartsWith(GuidTag, StringComparison.Ordinal);
            if (hasGuidTag && Guid.TryParse(value.Substring(GuidTag.Length, value.Length - GuidTag.Length), out var guid))
            {
                return guid;
            }
            return value;
        ...
        private void HandleTableEntryModified(SharedTableData.SharedTableEntry entry)
        {
