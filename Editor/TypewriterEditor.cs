...
namespace Aarthificial.Typewriter.Editor
{
    public class TypewriterEditor : EditorWindow, IHasCustomMenu
    {
        ...
        {
            Instance = this;
            titleContent = new GUIContent { text = "Typewriter", image = Resources.Load<Texture2D>("Textures/TypewriterWindow") };
        }
        ...
        public static void Open() => GetWindow<TypewriterEditor>();

        [MenuItem("Tools/Typewriter/Recreate Lookup")]
        private static void RecreateLookup() => TypewriterDatabase.Instance.CreateLookup();
        ...
        public int GetSelectedEntry() => _database.EntryProperty?.FindEntryID() ?? 0;
        ...
        public void AddItemsToMenu(GenericMenu menu) => menu.AddItem(new GUIContent("Recreate lookup"), false, RecreateLookup);
    }
}
