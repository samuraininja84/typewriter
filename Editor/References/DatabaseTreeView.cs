...
namespace Aarthificial.Typewriter.Editor.References
{
    ...
    public class DatabaseTreeView : TreeView
    {
        ...
        public EntryVariant FilterVariant
        {
            set
            {
              if (_filterVariant == value) return;
        ...
        public DatabaseTreeView(BaseEntry currentEntry, Action<BaseEntry> selectionHandler, EntryFilterAttribute filter) : base(new TreeViewState())
        {
            Filter = filter;
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            var id = 1;
            var emptyChild = new CollectionTreeViewItem(null, id++) { displayName = $"None ({Filter.GetName()})" };

            if (Filter.AllowEmpty) Root.AddChild(emptyChild);

            var tables = TypewriterDatabase.Instance.Tables;
            if (Filter.TableName != null) tables = tables.Where(table => table.TableName == Filter.TableName).ToList();
            ...
            foreach (var table in tables)
            {
                ...
                var entryLists = Filter.BaseType == null
                  ...
                  : types.Select(type => table.GetEntriesOfType(type)
                  .Where(entry => Filter.BaseType.IsInstanceOfType(entry))
                  .ToList()).ToList();

                var nonEmpty = types.Where((type, index) => entryLists[index].Count > 0).ToList();
                if (nonEmpty.Count == 0) continue;

                var isSingleType = nonEmpty.Count == 1;
                var group = new TreeViewItem(id++)
                {
                    displayName = isSingleType ? $"{table.name}/{nonEmpty[0]}" : table.name,
                    icon = _tableIcon
                };
                ...
                for (var index = 0; index < types.Count; index++)
                {
                    ...
                    var subGroup = isSingleType ? group : new TreeViewItem(id++) { displayName = type.ToString() };

                    foreach (var entry in entries)
                    {
                        if (entry == _currentEntry) _selectedId = id;
                        var item = new CollectionTreeViewItem(entry, id++) { displayName = entry.GetKey() };
                        subGroup.AddChild(item);
                    }

                    if (!isSingleType) group.AddChild(subGroup);
                }
            ...
            if (!Filter.AllowEmpty && firstEntry == null) Root.AddChild(emptyChild);
        ...
        private class CollectionTreeViewItem : TreeViewItem
        {
            ...
            {
                Entry = entry;
                if (entry != null && EntryTypeCache.TryGetDescriptor(entry.GetType(), out var descriptor))
                {
