...
namespace Aarthificial.Typewriter.Editor.References
{
  ...
  public class EntryReferencePropertyDrawer : PropertyDrawer
  {
    ...
    {
        var idProperty = property.FindPropertyRelative(nameof(EntryReference.InternalID));
        ...
        (
            ...
            id =>
            {
                ...
                if (recreateRelations) TypewriterUtils.RecreateRelations();
            },
            position,
            filter
        );
        ...
        if (idProperty.intValue != previousId && recreateRelations) TypewriterUtils.RecreateRelations();
    }

    protected virtual EntryFilterAttribute GetFilter()
    {
        var attributes = fieldInfo.GetCustomAttributes(typeof(EntryFilterAttribute), false);
        return attributes.Length > 0 ? (EntryFilterAttribute)attributes[0] : new EntryFilterAttribute();
    }

    public static int CollectionEntryPicker(Rect position, int previousId, Action<int> selectionHandler, Rect popupPosition, EntryFilterAttribute filter)
    {
        ...
        if (DropEntry(position, filter, out var dropEntry)) previousId = dropEntry.ID;
        ...
        if (DrawShowEntryButton(ref position, entry) && TypewriterEditor.Instance != null)
        {
            var selected = TypewriterEditor.Instance.GetSelectedEntry();
            if (selected != 0 && TypewriterDatabase.Instance.TryGetEntry(selected, out var selectedEntry) && filter.Test(selectedEntry))
            {
                previousId = selected;
                entry = selectedEntry;
        ...
        else
        {
            ...
            if (EntryTypeCache.TryGetDescriptor(entry.GetType(), out var descriptor))
            {
        ...
        if (EditorGUI.DropdownButton(position, label, FocusType.Passive))
        {
            ...
            (
                popupPosition,
                new DatabaseTreePopup(new DatabaseTreeView(entry, selection => selectionHandler(selection?.ID ?? 0), filter))
                {
    ...
    public static bool DropEntry(Rect position, EntryFilterAttribute filter, out BaseEntry entry)
    {
    ...
    public static bool DrawShowEntryButton(ref Rect position, BaseEntry entry)
    {
        ...
        if (GUI.Button(position, Styles.SelectIcon, Styles.IconButton) && entry != null)
        {
    ...
    {
        public static readonly GUIContent SelectIcon = EditorGUIUtility.IconContent("Animation.FilterBySelection");

        public static readonly GUIStyle IconButton = new("MiniButton") { padding = new RectOffset(0, 0, 0, 0) };
    }
  }
}
