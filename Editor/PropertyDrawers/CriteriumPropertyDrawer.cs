...
namespace Aarthificial.Typewriter.Editor.PropertyDrawers
{
    ...
    public class CriterionPropertyDrawer : PropertyDrawer
    {
        ...
        {
            var referenceProperty = property.FindPropertyRelative(nameof(BlackboardCriterion.FactReference));
            var minProperty = property.FindPropertyRelative(nameof(BlackboardCriterion.Min));
            var maxProperty = property.FindPropertyRelative(nameof(BlackboardCriterion.Max));
            var typeProperty = property.FindPropertyRelative(nameof(BlackboardCriterion.Operation));
            ...
            switch ((BlackboardCriterion.OperationType)typeProperty.enumValueIndex)
            {
                case BlackboardCriterion.OperationType.EqualReference:
                    var referenceValue = EntryReferencePropertyDrawer.CollectionEntryPicker
                    (
                        position,
                ...
                case BlackboardCriterion.OperationType.GreaterEqual:
                    var greaterEqualValue = EditorGUI.IntField(position, minProperty.intValue);
                    if (EditorGUI.EndChangeCheck())
                    {
                ...
                case BlackboardCriterion.OperationType.LessEqual:
                    var lessEqualValue = EditorGUI.IntField(position, maxProperty.intValue);
                    if (EditorGUI.EndChangeCheck())
                    {
                ...
                case BlackboardCriterion.OperationType.ClosedInterval:
                    position.width /= 2;
                    var minClosedValue = EditorGUI.IntField(position, minProperty.intValue);
                    position.x += position.width + 2;
                    position.width -= 2;
                    var maxClosedValue = EditorGUI.IntField(position, maxProperty.intValue);
                    if (EditorGUI.EndChangeCheck())
                    {
