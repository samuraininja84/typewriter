...
namespace Aarthificial.Typewriter.Editor.Descriptors
{
    ...
    public class RuleEntryDescriptor : EntryDescriptor
    {
        ...
        public override void CreatePreviousMenu(BaseEntry entry, List<string> names, List<int> ids, ref int current)
        {
            ...
            {
                if (!reference.TryGetEntry(out var trigger)) continue;  
                if (trigger.ID != 0 && TypewriterDatabase.Instance.TryGetTable(trigger.ID, out var table))
                {
                    names.Add(mainTable == table ? trigger.GetKey() : $"{table.name}/{trigger.GetKey()}");
                    ids.Add(trigger.ID);
                }
            }
        }

        public override void CreateAlternativeMenu(BaseEntry entry, List<string> names, List<int> ids, ref int current)
        {
            var rule = (RuleEntry)entry;
            TypewriterDatabase.Instance.TryGetTable(rule.ID, out var mainTable);
            foreach (var reference in rule.Triggers.List)
            {
                if (!reference.TryGetEntry(out var triggerEntry)) continue;
                foreach (var response in triggerEntry.Entries)
                {
                    ...
                    {
                        if (response.ID == rule.ID) current = names.Count;
