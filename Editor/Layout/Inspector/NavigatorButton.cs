...
namespace Aarthificial.Typewriter.Editor.Layout.Inspector
{
    public class NavigatorButton : VisualElement
    {
        ...
        public void Configure(IEnumerable<string> names, IEnumerable<int> ids, int current = -1)
        {
        ...
        private void Refresh()
        {
            ...
            if (_options.Count > 1)
            {
                ...
                if (_currentOption < 0) _dropdown.value = Title;
            ...
            {
                Add(_button);
                _button.SetEnabled(_currentOption < 0 ? _options.Count > 0 : _options.Count > 1);
            }
