...
namespace Aarthificial.Typewriter.Editor.Layout.Inspector
{
    public class Toolbar : BindableElement
    {
        ...
        public Toolbar()
        {
            var visualTree = Resources.Load<VisualTreeAsset>("UXML/Layout/Inspector/Toolbar");
            ...
            this.AddManipulator(new ContextualMenuManipulator(evt =>{ evt.menu.AppendAction($"Copy ID", _ => GUIUtility.systemCopyBuffer = _titleEntryId.text); }));
        ...
        public void BindCategory(SerializedProperty property)
        {
            ...
            if (_tableSo?.targetObject == table) return;
        ...
        {
            if (!property.Update(ref _property)) return;
            ...
            {
                _titleEntryId.text = property.FindPropertyRelative(nameof(BaseEntry.ID)).intValue.ToString();

        #if UNITY_LOCALIZATION
                var textProperty = property.FindPropertyRelative(nameof(LocalizedRuleEntry.Text));
                if (textProperty == null)
                {
                    _titleEntry.BindProperty(property.FindPropertyRelative(nameof(BaseEntry.Key)));
                }
                else
                {
    ...
    #if UNITY_LOCALIZATION
        private void HandleEntryChanged() => _titleEntry.text = _binding.Key;
    #endif
