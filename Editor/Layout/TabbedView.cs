...
namespace Aarthificial.Typewriter.Editor.Layout
{
    public class TabbedView : VisualElement
    {
        ...
        public TabbedView()
        {
            var visualTree = Resources.Load<VisualTreeAsset>("UXML/Layout/TabbedView");
            visualTree.CloneTree(this);
            _tabContainer = this.Q<VisualElement>("tabs");
            RegisterCallback<AttachToPanelEvent>(HandleAttachToPanel);
        }

        private void HandleAttachToPanel(AttachToPanelEvent evt) => Refresh();
        ...
        public void Refresh()
        {
            ...
            (
                element =>
                {
                    ...
                    if (element is EditableListView listView) tab.tooltip = listView.TitleTooltip;

                    _tabContainer.Add(tab);
                    element.style.display = tab.value ? DisplayStyle.Flex : DisplayStyle.None;
                    return tab;
                }
            )
            .ToList();

            if (SelectAny && !_tabs.Any(element => element.value)) ToggleTab(0, true);
    }
        ...
        {
            private readonly UxmlBoolAttributeDescription _selectAny = new() { name = "select-any" };
