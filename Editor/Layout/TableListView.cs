...
namespace Aarthificial.Typewriter.Editor.Layout
{
    public class TableListView : VisualElement
    {
        ...
        {
            var visualTree = Resources.Load<VisualTreeAsset>("UXML/Layout/TableListView");
            visualTree.CloneTree(this);

            _search = this.Q<ToolbarSearchField>("search");
            List = this.Q<SimpleList>("list");

            _search.RegisterValueChangedCallback(evt => List.SearchText = evt.newValue);
            List.ExtractValue = property => property.FirstString()?.stringValue ?? string.Empty;
            ...
            RegisterCallback<AttachToPanelEvent>(_ => { _search.value = EditorPrefs.GetString(this.GetViewDataKey() + "_search"); });
            RegisterCallback<DetachFromPanelEvent>(_ => EditorPrefs.SetString(this.GetViewDataKey() + "_search", _search.value));
        }
        ...
        private void CreateTable(Type type)
        {
            ...
            var index = List.Selection.Count > 0 ? List.Selection.OrderBy(element => element.Index).First().Index + 1 : -1;
        ...
        {
            if (_database == null || List.Selection.Count == 0) return;
            ...
            foreach (var data in List.Selection)
            {
                ...
                AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(data.Property.objectReferenceValue));
            }

            EditorUtility.SetDirty(_database);
            Refresh();
            if (index > 0) List.selectedIndex = index < _database.Tables.Count ? index : index - 1;
    }
        ...
        public void ClearSearch() => _search.value = null;
        ...
        private void HandleAddMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            foreach (var type in types) evt.menu.AppendAction(type.Name, _ => CreateTable(type));
        }
