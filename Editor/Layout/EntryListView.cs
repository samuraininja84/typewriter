...
namespace Aarthificial.Typewriter.Editor.Layout
{
    public class EntryListView : VisualElement
    {
        ...
        {
            var visualTree = Resources.Load<VisualTreeAsset>("UXML/Layout/EntryListView");
            ...
            _facts.AddManipulator(new ContextualMenuManipulator(evt => HandleEntryMenu(evt, EntryVariant.Fact)));
            _events.AddManipulator(new ContextualMenuManipulator(evt => HandleEntryMenu(evt, EntryVariant.Event)));
            _rules.AddManipulator(new ContextualMenuManipulator(evt => HandleEntryMenu(evt, EntryVariant.Rule)));
            ...
            {
                list.List.RegisterCallback<PointerDownEvent>(HandlePointerDown, TrickleDown.TrickleDown);
            ...
            creationManipulator.activators.Add(new ManipulatorActivationFilter { button = MouseButton.LeftMouse });
            add.AddManipulator(creationManipulator);
            remove.clicked += HandleRemove;
        }

        private string ExtractValue(SerializedProperty property) => property.FindEntry()?.GetKey() ?? string.Empty;
        ...
        private void HandleRemove() => RemoveEntries(_lists.SelectMany(list => list.List.Selection));

        private void HandlePointerDown(PointerDownEvent evt)
        {
            if (evt.button != 1 && (evt.button == 2 || evt.shiftKey || evt.ctrlKey)) return;
        ...
        private void HandleSelection(List<SimpleList.ItemData> _)
        {
            ...
            _selection.Clear();
            foreach (var list in _lists) _selection.AddRange(list.List.Selection);
        ...
        private void HandleAddMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            foreach (EntryVariant type in Enum.GetValues(typeof(EntryVariant)))
            {
                ...
                if (!first) evt.menu.AppendSeparator();
                first = false;
                foreach (var visibleType in types) evt.menu.AppendAction(visibleType.Name, _ => CreateEntry(visibleType.RealType));
            }
        }

        private void HandleEntryMenu(ContextualMenuPopulateEvent evt, EntryVariant variant)
        {
            if (TypewriterDatabase.Instance == null) return;
            ...
            if (visibleTypes.Count > 1)
            {
                ...
                {
                    evt.menu.AppendAction($"Create/{visibleType.Name}", _ => CreateEntry(visibleType.RealType));
                }
            }
            else if (visibleTypes.Count > 0)
            {
                evt.menu.AppendAction($"Create {visibleTypes[0].Name}",_ => CreateEntry(visibleTypes[0].RealType));
            }
            ...
            if (_selection.Count > 0)
            {
                ...
                {
                    evt.menu.AppendAction($"Move To/{table.name}", _ => MoveEntries(table));
                }
            }

            var list = evt.target as SimpleList;
            if (list == null && evt.target is ExpandableListView expandableList) list = expandableList.List;
            if (list != null && list.Selection.Count > 0) evt.menu.AppendAction("Remove", _ => RemoveEntries(list.Selection));
        }
        ...
        private void CreateEntry(Type entryType)
        {
            ...
            if (EntryTypeCache.TryGetDescriptor(entry, out var descriptor)) descriptor.HandleEntryCreated(entry, _table);
        ...
        private void MoveEntries(DatabaseTable table)
        {
            ...
            foreach (var data in _selection) TypewriterDatabase.Instance.MoveToTable(data.Property.FindEntryID(), table);
            Refresh();
        ...
        private void RemoveEntries(IEnumerable<SimpleList.ItemData> entries)
        {
            ...
            foreach (var itemData in entries) TypewriterDatabase.Instance.RemoveEntry(itemData.Property.FindEntryID());
            Refresh();
        }

        private void HandleGeometryChanged(GeometryChangedEvent _)
        {
            foreach (var list in _lists) list.MaxHeight = _container.layout.height;
        }
        ...
        public void Refresh()
        {
            ...
            _ignoreSelectionEvents = true;
            foreach (var list in _lists) list.List.Synchronize();
        ...
        public void Select(BaseEntry entry)
        {
            ...
            foreach (var list in _lists) list.List.ClearSelection();
