...
namespace Aarthificial.Typewriter.Editor.Lists
{
    public class EditableListView : BindableElement
    {
        ...
        private void HandleContextMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            if (List.Selection.Count > 0) evt.menu.AppendAction("Remove", _ => HandleRemove());
        }
        ...
        {
            if (_property?.serializedObject.targetObject == null) return;

            var index = List.Selection.Count > 0 ? List.Selection.Last().Index + 1 : _property.arraySize;
            ...
            List.Synchronize();
            if (index > -1 && index < List.Source.Count) List.selectedIndex = index;
        }
        ...
        private void HandleRemove()
        {
            ...
            {
                foreach (var index in indices) _property.DeleteArrayElementAtIndex(List.Source[index].Index);
                _property.serializedObject.ApplyModifiedProperties();
            }
            else
            {
            ...
            List.Synchronize();
            if (firstIndex > 0) List.selectedIndex = firstIndex - 1;
        }
        ...
        public new class UxmlTraits : BindableElement.UxmlTraits
        {
            ...
            private readonly UxmlEnumAttributeDescription<SelectionType> _selectionType = new() { name = "selection-type" };
            private readonly UxmlStringAttributeDescription _title = new() { name = "title" };
            private readonly UxmlStringAttributeDescription _tooltip = new() { name = "title-tooltip" };
