...
namespace Aarthificial.Typewriter.Editor.Lists.Items
{
    public class BaseEntryListItem : LabelListItem
    {
        ...
        {
            var dragHandle = new VisualElement { tooltip = "Drag onto an entry reference to assign it." };
        ...
        protected void SetType(SerializedProperty property)
        {
            ...
            if (TypewriterUtils.TryParseType(typeName, out var type) && EntryTypeCache.TryGetDescriptor(type, out var attribute))
            {
        ...
        protected void SetID(SerializedProperty property) => ID = property.FindPropertyRelative(nameof(BaseEntry.ID))?.intValue ?? 0;
