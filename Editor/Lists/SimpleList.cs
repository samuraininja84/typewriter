...
namespace Aarthificial.Typewriter.Editor.Lists
{
    public class SimpleList : ListView
    {
        ...
        public Func<SerializedProperty, string> ExtractValue = property => property.FirstString()?.stringValue ?? string.Empty;
        ...
        public SimpleList()
        {
            ...
            this.Q<ScrollView>().RegisterCallback<PointerDownEvent>
            (
                  evt =>
                  {
                      if (evt.button != 1) return;

                      var localPosition = (Vector2)evt.localPosition + _scrollView.scrollOffset;
                      var num = (int)(localPosition.y / (double)fixedItemHeight);
                      if (num < Source.Count && !selectedIndices.Contains(num)) SetSelection(num);
                  }
            );
            ...
            (
                _ =>
                {
                    ...
                    if (_property != null) SelectInitial();
                }
            );

            RegisterCallback<DetachFromPanelEvent>(_ => EditorPrefs.SetString(this.GetViewDataKey(), string.Join(",", selectedIndices)));
        }
        ...
        private void HandleItemIndexChanged(int from, int to)
        {
            ...
            if (selectedIndices.Contains(from)) SetSelectionWithoutNotify(selectedIndices.Where(id => id != from).Append(to).ToList());
            Synchronize();
        }

        private void HandleContextMenu(ContextualMenuPopulateEvent evt) => evt.menu.AppendAction("Refresh", _ => { Synchronize(); });

        private void BindItem(VisualElement item, int id)
        {
            if (item is EditableListItem editableListItem && id >= 0 && Source.Count > id)
            {
        ...
        {
          if (item is EditableListItem editableListItem) editableListItem.Unbind();
        }

        private VisualElement MakeItem() => CreateItem();

        private void SelectInitial()
        {
            if (_initialSelection == null) return;
            SetSelection(_initialSelection.Where(index => index < Source.Count && index >= 0));
        ...
        public void Synchronize()
        {
            ...
            if (style.flexGrow.value == 0) style.height = Source.Count * fixedItemHeight;
        ...
        private void FillItems()
        {
            ...
            for (var i = 0; i < _property.arraySize; i++)
            {
                ...
                if (!isEmpty)
                {
                    ...
                    if (value.IndexOf(_searchText, StringComparison.OrdinalIgnoreCase) < 0) continue;
                }

                Source.Add(new ItemData { Index = i, Property = property });
            }
        }

        public void BindProperty(SerializedProperty property)
        {
            if (property.Update(ref _property)) Synchronize();
        }
        ...
        private void UpdateSelection()
        {
            ...
            {
                if (index >= 0 && Source.Count > index) Selection.Add(Source[index]);
            }
