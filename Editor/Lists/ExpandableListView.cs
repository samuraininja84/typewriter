...
namespace Aarthificial.Typewriter.Editor.Lists
{
    public class ExpandableListView : VisualElement
    {
        ...
        {
            var visualTree = Resources.Load<VisualTreeAsset>("UXML/Lists/ExpandableListView");
            ...
            (
                evt =>
                {
                    if (!evt.newValue) List.ClearSelection();
                    CalculateLayout();
                }
            );
        }
        ...
        {
            if (selection.Count > 0) Foldout.value = true;
        }

        private void Synchronize()
        {
            style.display = List.Source.Count > 0 ? DisplayStyle.Flex : DisplayStyle.None;
            CalculateLayout();
        }
        ...
        private void CalculateLayout()
        {
            ...
            style.minHeight = new Length(Mathf.Min(height / _maxHeight * 100, 33.3333f),LengthUnit.Percent);
        }

        public void BindProperty(SerializedProperty property) => List.BindProperty(property);

        public new class UxmlFactory : UxmlFactory<ExpandableListView, UxmlTraits> { }
